#!/usr/bin/env python3
"""
Simple vCard Viewer - Browse t        # Search frame
...
class VCardViewer:
    ...
    def setup_ui(self):
        ...
        # Enable keyboard navigation in main window
    ...
    def on_contact_select(self, event=None):
        ...
        if selection and self.filtered_vcards:
            # For single selection, update the preview
            if len(selection) == 1:
            ...
            if len(selection) > 1:
                self.delete_btn.config(text=f"🗑️ Delete ({len(selection)})")
            else:
                self.delete_btn.config(text="🗑️ Delete")

    def on_listbox_key(self, event):
        """Handle key presses in the listbox"""
        if event.keysym in ['Up', 'Down']:
            # Let the default listbox behavior handle the selection change
            self.root.after(10, self.on_contact_select)  # Delay to let selection update
        elif event.keysym == 'Delete':
            self.delete_contacts()
        elif event.keysym == 'Return':
            self.on_contact_double_click()

    def on_global_key(self, event):
        """Handle global keyboard shortcuts"""
        # Only process if not in an entry widget or text editor
        if isinstance(event.widget, (tk.Entry, tk.Text)):
            return
            
        if event.keysym in ['Up', 'Down', 'Left', 'Right']:
            # Arrow key navigation when listbox doesn't have focus
            if event.widget != self.contact_listbox:
            ...
            if event.widget != self.contact_listbox:
                self.delete_contacts()
        elif event.char.lower() == 's' and event.state & 0x4:  # Ctrl+S
            self.save_as_file()
        elif event.char.lower() == 'f' and event.state & 0x4:  # Ctrl+F
    ...
    def delete_contacts(self):
        ...
        # Get selected contacts from listbox
        selection = self.contact_listbox.curselection()
        if not selection:
            # No selection in listbox, delete current contact
            if self.current_index < len(self.vcards):
                selection = [self.filtered_vcards.index(self.vcards[self.current_index])]
            else:
                return
        
        # Get the actual vCard objects to delete
        contacts_to_delete = []
        names_to_delete = []
        
        for list_index in selection:
            if 0 <= list_index < len(self.filtered_vcards):
                card = self.filtered_vcards[list_index]
                contacts_to_delete.append(card)
                
                # Get name for confirmation
                fn = getattr(card, 'fn', None)
                name = fn.value.strip() if fn else f"Contact {self.vcards.index(card) + 1}"
        ...
        if len(contacts_to_delete) == 1:
            message = f"Are you sure you want to delete '{names_to_delete[0]}'?"
        else:
            message = f"Are you sure you want to delete {len(contacts_to_delete)} contacts?\n\n"
            if len(names_to_delete) <= 5:
                message += "Contacts to delete:\n• " + "\n• ".join(names_to_delete)
            else:
                message += "Contacts to delete:\n• " + "\n• ".join(names_to_delete[:5])
                message += f"\n• ... and {len(names_to_delete) - 5} more"
        
        if messagebox.askyesno("Delete Contact(s)", message):
            # Remove from main list (in reverse order to maintain indices)
            for card in reversed(contacts_to_delete):
    ...
    def delete_contact(self):
        """Legacy method - redirects to delete_contacts for compatibility"""
        self.delete_contacts()
    ...
    def update_navigation(self):
        ...
        if self.vcards and hasattr(self, 'contact_listbox'):
            ...
            try:
                ...
                # Only update if it's a single selection scenario
                if len(self.contact_listbox.curselection()) <= 1:
